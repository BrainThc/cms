...
K.options = {
	...
	htmlTags : {
		...
		'span,p,ol,ul,li,blockquote,h1,h2,h3,h4,h5,h6,dl,dt,dd,b,strong,i,em' : [
...
function _mediaEmbed(attrs) {
	...
	_each(attrs, function(key, val) {
		if(val == 'video/x-ms-asf-plugin') is_video = 1;
		html += key + '="' + val + '" ';
	});

	if(is_video){
		html = '<video controls="controls" '+html+'>您的浏览器不支持 video 标签。</video>';
	}else{
		html = '<embed '+html+' />';
	}
