...
function up_images(iframeWin, topWin,id,inputid){ 
		...
		if(num){
			iframeWin.$('#myuploadform  div ').each(function(){
					...
					data += '<div style="display:inline-block;" id="uplist_'+aid+'">'+aidinput+'<img width="150" height="150" src="'+src+'" /><input type="hidden" size="50" class="input-text" name="'+inputid+'[]" value="'+src+'"  />  <p><input type="text" class="input-text" name="'+inputid+'_name[]" value="'+name+'" size="30" /> &nbsp;<a href="javascript:remove_this(\'uplist_'+aid+'\');">移除</a> </p></div>';
			});			
