...
class CartAction extends BaseAction{
    ...
    {	
		...
		$cart = $this->dao->where("sessionid='{$this->sessionid}' or userid = '{$this->_userid}' ")->select();
	...
	public function getnum(){ 
		$where = 'sessionid='.intval($this->sessionid).' or userid = '.intval($this->_userid);
		$num = $this->dao->where($where)->count();
		echo json_encode($num);
	}
	public function clear(){
		$this->dao->where("sessionid = '$this->sessionid' or userid = '$this->_userid'")->delete();
