...
class BaseAction extends Action
{
    ...
    public function index($catid='',$module='')
    {
		...
		if($module=='Guestbook'){
			...
			if($catid){
				...
				$where = " status=1 ";
				if($keyword = get_safe_replace($_GET['keyword'])) $where .=' and title like "%'.trim($keyword).'%" ';
				if($cat['child']){							
	...
	{
		if($_REQUEST['isuser'] == '1'){
			if(!$this->_userid){
				$this->error('请先登录后再下载..',U('Login/index'));
			}
		}
		$module = $module ? $module : MODULE_NAME;
		$id = $id ? $id : intval($_REQUEST['id']);
