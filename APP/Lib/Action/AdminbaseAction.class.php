...
class AdminbaseAction extends Action
{
		...
		if(APP_LANG){
			...
			if($_GET['l']){
				if($this->Lang[$_GET['l']]['id']){
					$_SESSION['TP_lang'] = $_GET['l'];
					$_SESSION['TP_langid'] = $this->Lang[$_GET['l']]['id'];
				}else{
					$this->error ( L ( 'NO_LANG' ) );
				}
			}elseif(!$_SESSION['TP_lang'] || !$_SESSION['TP_langid']){
				$_SESSION['TP_lang'] = $this->sysConfig['DEFAULT_LANG'];
				$_SESSION['TP_langid'] = $this->Lang[$this->sysConfig['DEFAULT_LANG']]['id'];
			}
			define('LANG_NAME',$_SESSION['TP_lang']);
			define('LANG_ID',$_SESSION['TP_langid']);
	...
	function _list($modelname, $map = '', $sortBy = '', $asc = false ,$listRows = 15) {
		...
		if ($count > 0) {
			...
			$page->urlrule = U($modelname.'/index', array_filter($_GET));
			//分页显示
			$page = $page->show ();
			//列表排序显示
			$sortImg = $sort; //排序图标
		...
		return array('list'=>$voList,'page'=>$page);
	}
	...
	public function create_show($id,$module)
    {
		...
		if(APP_LANG){
			...
			$T = F('config_'.LANG_NAME,'',APP_PATH.'Tpl/Home/'.$this->sysConfig['DEFAULT_THEME'].'/');
		}else{
		...
		if($CONTENT_POS !== false){
				...
				foreach ($pageurls as $p=>$urls) {
					...
					$this->buildHtml($filename,$dir,APP_PATH.'Tpl/Home/'.$this->sysConfig['DEFAULT_THEME'].'/'.$template.C('TMPL_TEMPLATE_SUFFIX'));
				}
		}else{
		...
		if($cat['moduleid']==1){
			...
			if($CONTENT_POS !== false){
					...
					foreach ($pageurls as $p=>$urls) {
						...
						$r=$this->buildHtml($filename,$dir,APP_PATH.'Tpl/Home/'.$this->sysConfig['DEFAULT_THEME'].'/Page_'.$template.C('TMPL_TEMPLATE_SUFFIX'));
					}
			}else{
					$this->assign($data);
			...
			if($cat['listtype']==1){
				...
				if($count){
					...
					$list = $dao->field($field)->where($where)->order('listorder asc,id desc')->limit($page->firstRow . ',' . $page->listRows)->select();
					$this->assign('pages',$pages);
					$this->assign('list',$list);
				}
				$template_r = 'list';
			...
			$r=$this->buildHtml($filename,$dir,APP_PATH.'Tpl/Home/'.$this->sysConfig['DEFAULT_THEME'].'/'.$cat['module'].'_'.$template.C('TMPL_TEMPLATE_SUFFIX'));
	...
	public function create_index($sitemap)
    {
		...
		if(empty($sitemap)){
			...
			$this->assign('bcid',0);
			$r=$this->buildHtml('index','./'.$lang,APP_PATH.'Tpl/Home/'.$this->sysConfig['DEFAULT_THEME'].'/Index_index'.C('TMPL_TEMPLATE_SUFFIX'));
		}else{
			$this->assign('sitemap','1');
			$r=$this->buildHtml('sitemap','./'.$lang,APP_PATH.'Tpl/Home/'.$this->sysConfig['DEFAULT_THEME'].'/Sitemap'.C('TMPL_TEMPLATE_SUFFIX'));
		}
		if($r) return true;
    }
