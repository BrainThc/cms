...
class CategoryAction extends AdminbaseAction
{
    ...
    public function index()
    {
        if($this->categorys){
			foreach($this->categorys as $r) {
				...
				$r['str_manage'] .= '<a href="'.U('Category/add',array( 'parentid' => $r['id'],'type'=>$r['type'])).'">'.L('add_catname').'</a> | <a href="'.U('Category/edit',array( 'id' => $r['id'],'type'=>$r['type'])).'">'.L('edit').'</a> ';
				if($r['isdel'] != '1'){
					$r['str_manage'] .= '| <a href="javascript:confirm_delete(\''.U('Category/delete',array( 'id' => $r['id'])).'\')">'.L('delete').'</a> ';
				}
	...
	public function _before_add()
    {
		...
		$thinkphp_auth = authcode('1-1-0-1-jpeg,jpg,png,gif-3-0', 'ENCODE',$thinkphp_auth_key);
    ...
    public function insert()
    {
		...
		*/
		...
		$catdir = $_POST['catdir'];
		$rs = $this->dao->where('parentid = '.$parentid.' and catdir = '.$catdir)->field('id')->find();
		if($rs){
			$maxcatdir = $this->dao->where('parentid = '.$parentid)->max('catdir');
			$_POST['catdir'] = $maxcatdir+1;
		}
		...
		if(is_array($this->categorys)) {
			foreach($this->categorys as $id => $cat) {
				...
				if( $cat['type']==1){
					...
					//判断是否默认到子栏目
					if($cat['ischild'] && $cat['child']) {
						$childs = explode(',',$cat['arrchildid']);
						if($childs[1]) $cat = $this->categorys[$childs[1]];
					}
					if($cat['type'] == 1){
						$url = $cat['url'];
					}else{
