...
class UpgradeAction extends AdminbaseAction {
	...
	private $_filearr = array('thinkphp', 'Public', 'Core', '');
	private $_url = 'http://download.thinkphp.cn/';
		...
		if(!empty($_GET['do'])) {
			...
			foreach($patchlist as $k=>$v) {		 
				...
				if($this->copyfileerror){
					...
					if(file_exists($upgradezip_source_path.'/thinkphp.sql')){
						$sqldata = file_get_contents($upgradezip_source_path.'/thinkphp.sql');
	...
	public function checkfile() {
			...
			$v=explode(' ',VERSION);
			$var=$v[0];
			$thinkphp_filemd5 = @file_get_contents($this->_url.$var.'/md5file/'.UPDATETIME.".php");
			$thinkphp_filemd5_arr = json_decode($thinkphp_filemd5, 1);unset($thinkphp_filemd5);

			$filemd5_arr_key = array_keys($this->filemd5_arr);
			$thinkphp_filemd5_arr_key = array_keys($thinkphp_filemd5_arr);

			$editfiles = array_diff($thinkphp_filemd5_arr, $this->filemd5_arr);
			$lostfile = array_diff( $thinkphp_filemd5_arr_key,$filemd5_arr_key);
			$unknowfile = array_diff($filemd5_arr_key, $thinkphp_filemd5_arr_key);
