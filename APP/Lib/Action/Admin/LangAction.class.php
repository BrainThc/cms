...
class LangAction extends AdminbaseAction {
	...
	function insert() {
		...
		if ($id !==false) {
			$db=D('');
			$db =   DB::getInstance();
			$db->execute("INSERT INTO `thinkphp_config`  (`varname`,`info`,`groupid`,`value`,`lang`) VALUES ('site_name','网站名称','2','','".$id."'),
			('site_url','网站网址','2','','".$id."'),
			('logo','网站LOGO','2','./Public/Images/logo.png','".$id."'),
			('site_email','站点邮箱','2','admin@thinkphp.cn','".$id."'),
			('seo_title','网站标题','2','','".$id."'),
