...
class ConfigAction extends AdminbaseAction {
	...
	public function index() {
		...
		foreach($config as $key=>$r) {
			if($r['groupid']==1 && $r['lang']==intval(LANG_ID)) $this->user_config[$r['varname']]=$r;
			if($r['groupid']==2){
				if(APP_LANG){
		...
		$thinkphp_auth = authcode('1-1-0-10-jpeg,jpg,png,gif-5-230', 'ENCODE',$thinkphp_auth_key);
	...
	public function dositeyulia() {
		$status = $_REQUEST['updatepass_stauts'];
		$data['value']=$status;
		$map['varname']=array("eq","updatepass_stauts");
		$f = $this->dao->where($map)->save($data);
	}
}
?>