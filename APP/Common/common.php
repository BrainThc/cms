...
function sendmail($site_email,$title,$html,$config=''){
	if(!$config)$config = F('Config');
	import ( '@.ORG.Smtp' );
	$smtpserver =$config['mail_server'];// "smtp.126.com";//SMTP服务器 
	$smtpserverport = $config['mail_port'];//25;//SMTP服务器端口 
	$smtpusermail = $config['mail_from'];//"ztlong2@126.com";//SMTP服务器的用户邮箱 
	$smtpuser = $config['mail_user'];//"ztlong2";//SMTP服务器的用户帐号 
	$smtppass =$config['mail_password'];// "1133220";//SMTP服务器的用户密码 
	
	$smtpemailto = $site_email;//发送给谁 
	$mailsubject = "=?UTF-8?B?".base64_encode($title)."?=";//邮件主题 
	$mailbody = $html;//邮件内容 
	$mailtype = "HTML";//邮件格式（HTML/TXT）,TXT为文本邮件 
	/*mail发送*/
	if($config['mail_type']==2){
		$mailbody = html_entity_decode(html_entity_decode($html));
		$from = $smtpusermail;
		$headers = 'Content-type: text/html; charset=utf-8';
		$r = mail($smtpemailto,$mailsubject,$mailbody,$headers);
		$k = $r?1:0;	
	}else{
		$smtp = new smtp($smtpserver,$smtpserverport,true,$smtpuser,$smtppass);//这里面的一个true是表示使用身份验证,否则不使用身份验证. 
		$smtp->debug = $debug;//是否显示发送的调试信息 
		$k=$smtp->sendmail($smtpemailto, $smtpusermail, $mailsubject, $mailbody, $mailtype,"",'',$headers); 
...
function checkfield($fields,$postdata){
		foreach ( $postdata as $key => $val ) {
				...
				if($setup['multiple'] || $setup['inputtype']=='checkbox' || $fields[$key]['type']=='checkbox'){
					...
					if(!is_array($postdata[$key])) $postdata[$key] =  safe_replace(strip_tags($postdata[$key]));
...
function order_pay_status($sn,$value){
	...
	if(substr($sn,0,2) == 'cz'){
		...
		if($value==2){
			...
			$r = M('User')->where('id='.intval($vo['userid']))->setInc('amount',intval($vo['price']));
		}
   	}else{
...
function  sql_split($sql,$tablepre) {
	...
	if($tablepre != "thinkphp_") $sql = str_replace("thinkphp_", $tablepre, $sql);
...
//以下自定义文件
//自定义栏目
function column($Categorys,$catid=0,$id=0,$par=''){
		 if($parentid=$Categorys[$catid]['arrparentid']){
			$catids=explode(',',$parentid);
		  }
		 if(!$catids[$id]){$catids[$id]=$catid;}
		if($par){
			 return $Categorys[$catids[$id]][$par] ;
		}else{
			return $catids[$id];
		}
}
...
function filepath($data,$leng=0,$id=0){
if(!empty($data)){
$p_data=explode(':::',$data);
$data=array();
$i=0;
	foreach($p_data as $k=>$res){
		...
		$data[$k]['filepath'] = $p_data_arr[0];
		$data[$k]['filename'] = $p_data_arr[1];
		}
		$i++;
	}
	if($id){
		$id=$id-1;
		return $data[$id]['filepath'];
	}
}
return $data;
}
//数值最低最高
function maximin($str,$min=1,$max=5){
	return $str<$min?$min:($str>$max?$max:$str);
}
//搜索路径处理
function getpath($value,$name,$str='?m=Search',$fuhao='&'){
	$path=$_GET;
	$path[$name]=$value;
	foreach($path as $k=>$v){
		$str.=$fuhao.$k.'='.$v;
...
//自定义图片宽高
function imgwh($f,$wh=false,$w=230,$h=230){
	if(!strstr($f,'://')){
		$f= '.'.str_replace(__ROOT__,'',$f);
	}
	list($fw, $fh, $tmp) = getimagesize($f);
...
//自定义默认图片
function defaimg($src='nopic.jpg'){
return "this.src='".__ROOT__."/Public/Images/".$src."'";
}

function ajaximg($src='msg_loading_d.gif'){
	return __ROOT__."/Public/Images/".$src;
}
...
function urlid($url){
	...
	if($query){
		if($data = explode('&',$url)){
			foreach($data as $r){
				list($name, $id) = explode('=',$r);
				if($name=='id'){ return intval($id); break;}
			}
		}
	}
}

function is_explode($str,$ex=',',$re = false){
	if($ex==',')$str = str_replace("，",",",$str);
	$array = explode($ex,$str);
	return is_int($re)?$array[$re]:$array;
}
...
function MD($model,$where,$field = '',$order=''){
	$rs = M($model)->where($where)->field($field)->order($order)->find();
	if($field){
		$field = is_explode($field);
		if(count($field) == 1){
			return $rs[$field[0]];
		}
	}
...
/**
* is_child 是否存在子栏目函数
* 参数1：当前栏目id,参数2：待查询的数组或逗号分割的字符串，参数三：将返回的字符串
*/
function is_child($catid,$arrchildid,$str=true){
	if(!is_array($arrchildid)){
		$arrchildid = is_explode($arrchildid);
	}
	if(in_array($catid,$arrchildid)) return $str;
}

/*
*模拟post提交函数
*参数1：要提交的地址(URL地址),参数二:要提交的数组
*/	
...
function get($k,$v,$data){
	if(!$data) $data = $_GET;
	$data[$k] = $v;
	return $data;
}

function gname($url,$n = 2){
	$data = explode('/', $url);
	return $data[$n];
}
...
function xurl($data){
	...
	if(!is_array($data)){
		$id = intval($data);
		return U('show',array('id'=>$id));
	}

	//传入的是数组
	if($data['ischild'] == 1){
		
		$ids = explode(',', $data['arrchildid']);
		return U($data['module'].'/index',array('id'=>$ids[1]));

	}else if($data['type'] == 1){
		return $data['url'];
	}else{
		return U($data['module'].'/index',array('id'=>$data['id']));
	}
}
