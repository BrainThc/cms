 ...
 *
 */
if(defined('ThinkPHP')!='ThinkPHP' && !defined("ThinkPHP"))  exit("Access Denied");
class TagLibTp extends TagLib
{
	protected $tags   =  array(
		...
		'subcat'=>array('attr'=>'catid,type,self,key,id,limit','level'=>3),
	...
	public function _subcat($attr,$content) {
		...
		$parsestr .= '<?php if('.$limit.'!=0 && $'.$key.' > '.$limit.') break;?>';
		$parsestr .= $content;
		$parsestr .= '<?php endif;?>';
		...
		$space		= !empty($tag['space']) ? $tag['space'] : '';
 		if(is_numeric($tag['catid'])){
			...
			foreach($arrparentid as $cid) {
				$parsestr[] = '<a href="'.$category_arr[$cid]['url'].'" '.($catid == $cid?'class="this"':'').'>'.$category_arr[$cid]['catname'].'</a>';
			}unset($category_arr);
			...
			$parsestr .= '$parsestr[] = \'<a href="\'.$Categorys[$cid][\'url\'].\'" \'.($catid == $cid?\'class="this"\':"").\' >\'.$Categorys[$cid][\'catname\'].\'</a>\';?>';
