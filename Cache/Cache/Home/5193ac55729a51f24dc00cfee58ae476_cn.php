...
<!--载入主体--> 
<!--banner-->
    <div class="inside_banner" style="background: url('__STYLE__/img/s1.jpg') no-repeat center;height:400px;">
    </div>
    <!---->
    <div class="question">
      <div class="wrap-1200">

        <div class="price pro-question">
          <ul>
          <?php if(is_array($list)): $k = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><a href="javascript:void(0)">
              <li class="<?php if($k==1) : ?> price_curr <?php endif;?>"><?php echo ($vo["title"]); ?></li>
            </a>
            <span style="display: block;"><?php echo ($vo["content"]); ?></span><?php endforeach; endif; else: echo "" ;endif; ?>
          </ul>
        </div>
      </div>
 	   ...
 	   <div class="opinion" id="question">
        <div class="wrap-1200">
        <form name="myform" id="myform" action="<?php echo U('Home/Index/insert');?>" method="post" onsubmit="return checkform(this);">
         
          <div class="opinion_title">顾客意见 <span>Customer opinion</span> </div>
          <p>
            <input type="text" id="username" name="username" class=" opinion_text" placeholder="姓名/ 昵称">
          </p>
          <p>
            <input type="text" id="Tel" name="Tel" class=" opinion_text" placeholder="联系方式">
          </p>
          <p>
            <textarea cols="" id="content" name="content" class=" opinion_text2" rows="" placeholder="问题或意见："></textarea>
          </p>
          <input type="submit" class="opinion_btn" value="提交">
      ...
      {
        if(nForm.username.value==""||nForm.username.value=="姓名或昵称")
        {
          alert("请留下您的姓名或昵称!");
  ...
  <script type="text/javascript">
          $('.pro-question a').click(function(){

        $('.pro-question li').removeClass('price_curr')
        $('.pro-question span').stop().slideUp()
        $(this).find('li').addClass('price_curr')
        $(this).next('span').stop().slideDown()
      })
  </script>
